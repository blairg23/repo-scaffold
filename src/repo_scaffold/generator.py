...
1. Copy `.env.example` to `.env` and set `GH_TOKEN`, `GITHUB_ORG`, and `GITHUB_REPO`.
2. Run `scripts/gh-create-project.sh` (or create the repo manually).
3. After pushing, run `scripts/gh-apply-settings.sh`.
4. Open issues with `.github/ISSUE_TEMPLATE/` forms and submit PRs with the PR template.

`gh-apply-settings.sh` applies merge/branch protection settings and also attempts to enable
Dependabot alerts + automated security updates (best-effort; warnings if unavailable).
...
If you use `GH_TOKEN`/`GITHUB_TOKEN` instead of `gh auth login`, ensure the token has the permissions needed by your workflow.

Recommended classic PAT scopes:

- `repo`
- `workflow` (required when pushing workflow files under `.github/workflows/`)
- `read:org` (often required for org repos)
  ...
  if [ "$kind" = "issue" ]; then
    ...
    if [ -n "$milestone" ]; then
      if parent_epic_number="$(issue_number_exact "$milestone")"; then
        body_payload="${body_payload}"$'\\n\\n'"Parent epic: #$parent_epic_number"$'\\n'
      else
        parent_lookup_rc=$?
        if [ "$parent_lookup_rc" -eq 2 ]; then
...
for epic in epics:
    ...
    labels = validate_str_list(epic.get("labels", ["epic"]), "epic.labels")
...
def build_scaffold_files(config: ScaffoldConfig) -> list[ScaffoldFile]:
    ...
    files: list[ScaffoldFile] = [
        ...
        ScaffoldFile(config.out_dir / "backlog" / "issues.json", _render_backlog_issues_json()),
        ...
        ScaffoldFile(
            ...
            _render_gh_create_project_script(config.name),
            executable=True,
        ),
        ScaffoldFile(
            config.out_dir / "scripts" / "create-issues.sh",
