...
# ---------------------------------------------------------------------------
# Actions template (written to .github/workflows/ by project setup)
# Use __UPPER_SNAKE__ placeholders; substituted via str.replace(), NOT .format().
# ---------------------------------------------------------------------------

_ISSUE_STATUS_SYNC_TEMPLATE = """\
# Generated by repo-scaffold project setup. Regenerate with:
#   poetry run repo-scaffold project setup --project-title "__PROJECT_TITLE__" --repo OWNER/REPO
name: Sync issue status with branch/PR lifecycle
...
jobs:
  sync-issue-status:
    ...
    env:
      ...
      BRANCH_STATUS: "__BRANCH_CREATED_STATUS__"
      PR_STATUS: "__PR_OPENED_STATUS__"
    steps:
      - name: Sync issue status
        ...
        run: |
          ...
          branch_ref = os.environ.get("BRANCH_REF", "")
          pr_head    = os.environ.get("PR_HEAD_REF", "")
          token      = os.environ["GH_TOKEN"]
          owner      = os.environ["OWNER"]
          repo       = os.environ["REPO"]
          proj_title = os.environ["PROJECT_TITLE"]

          if event == "create" and ref_type == "branch":
              branch = branch_ref
              target = os.environ["BRANCH_STATUS"]
          elif event == "pull_request":
              branch = pr_head
              target = os.environ["PR_STATUS"]
          else:
              sys.exit(0)

          m = re.search(r"(?:^|[/-])(?:issue-?)?(\\d+)(?:[/-]|$)", branch)
          if not m:
              print(f"No issue number in: {branch}", file=sys.stderr)
              sys.exit(0)
          ...
          data = gql(
              "query($o:String!){user(login:$o){projectsV2(first:50){nodes{id title}}}"
              " organization(login:$o){projectsV2(first:50){nodes{id title}}}}",
              {"o": owner},
          )
          project_id = None
          for ns in ("user", "organization"):
              for n in (data.get(ns) or {}).get("projectsV2", {}).get("nodes", []):
                  if isinstance(n, dict) and n.get("title") == proj_title:
                      project_id = n["id"]
                      break

          if not project_id:
              print(f"Project not found: {proj_title}", file=sys.stderr)
              sys.exit(0)

          try:
              issue_resp = urllib.request.urlopen(
                  urllib.request.Request(
                      f"https://api.github.com/repos/{repo}/issues/{issue_num}",
                      headers=headers,
                  )
              )
              issue_node_id = json.loads(issue_resp.read()).get("node_id")
          except Exception as exc:
              print(f"Issue #{issue_num} not found: {exc}", file=sys.stderr)
              sys.exit(0)

          gql(
              "mutation($p:ID!,$i:ID!){addProjectV2ItemById(input:{projectId:$p,contentId:$i}){item{id}}}",
              {"p": project_id, "i": issue_node_id},
          )
          ...
          item_id = next(
              (i["id"] for i in node.get("items", {}).get("nodes", [])
               if isinstance(i, dict) and (i.get("content") or {}).get("number") == issue_num),
              None,
          )
          if not item_id:
              print(f"Issue #{issue_num} not in project", file=sys.stderr)
              sys.exit(0)

          status_field = next(
              (f for f in node.get("fields", {}).get("nodes", [])
          ...
          option_id = next(
              (o["id"] for o in status_field.get("options", [])
               if isinstance(o, dict) and o.get("name") == target),
              None,
          )
          if not option_id:
              print(f"Status option not found: {target}", file=sys.stderr)
              sys.exit(0)
          ...
          gql(
              ...
              {"p": project_id, "i": item_id, "f": status_field["id"], "o": option_id},
          )
          print(f"Issue #{issue_num} -> {target}")
          PYEOF
"""


def _render_actions_template(
    project_title: str,
    branch_status: str,
    pr_opened_status: str,
) -> str:
    return (
        _ISSUE_STATUS_SYNC_TEMPLATE.replace("__PROJECT_TITLE__", project_title)
        .replace("__BRANCH_CREATED_STATUS__", branch_status)
        .replace("__PR_OPENED_STATUS__", pr_opened_status)
    )


def _write_actions_template(
    repo_dir: Path,
    project_title: str,
    branch_status: str,
    pr_opened_status: str,
    ...
    dest = workflows_dir / "issue-status-sync.yml"
    dest.write_text(
        _render_actions_template(project_title, branch_status, pr_opened_status),
        encoding="utf-8",
    )
    out(f"Written: {dest.relative_to(repo_dir)}")
    return dest
...
def setup_project(
    *,
    ...
    write_actions_template: bool = True,
    repo: str | None = None,
    out: Callable[[str], None] = print,
) -> ProjectMutationSummary:
    """Configure a project's status field and automation workflows from config."""
    from .github_api import (
        WORKFLOW_NAME_HINTS,
    ...
    search_dir = config_path.parent if config_path else repo_dir
    cfg = load_config(search_dir)

    # --- 1. Status field --------------------------------------------------
    status_options = [
        {"name": s.name, "color": s.color, "description": ""} for s in cfg.statuses
    ]
    cp = setup_project_status_field(project.id, token, options=status_options)
    if cp.returncode != 0:
        raise RuntimeError(
            cp.stderr.strip() or "Failed setting up project status field."
        )
    ...
    if cp.returncode != 0:
        ...
        try:
            workflows = json.loads(cp.stdout).get("workflows", [])
        ...
        for workflow in workflows:
            if not isinstance(workflow, dict):
                continue
            name_lower = (workflow.get("name") or "").lower()
            wf_id = workflow.get("id", "")
            for event_key, hints in WORKFLOW_NAME_HINTS.items():
                if any(h in name_lower for h in hints):
                    target_status = cfg.workflows.get(event_key, "")
                    if not target_status:
                        continue
                    cp2 = enable_project_workflow(wf_id, token, enabled=True)
                    if cp2.returncode == 0:
                        out(
        ...
        if not enabled_count:
            out(
                "Note: no native GitHub Projects workflows matched (they may already be enabled)."
            )

    # --- 3. Actions template (branch_created, pr_opened) ------------------
    if write_actions_template and repo_dir.exists():
        branch_status = cfg.workflows.get("branch_created", "In Progress")
        pr_opened_status = cfg.workflows.get("pr_opened", "To Review")
        _write_actions_template(
            repo_dir=repo_dir,
            project_title=project.title,
            branch_status=branch_status,
            pr_opened_status=pr_opened_status,
            out=out,
        )
